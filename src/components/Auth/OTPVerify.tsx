...
const OTPVerify: React.FC<OTPVerifyProps> = ({ setIsOTPSuccessful }) => {
  ...
  const handleChange = (index: number, value: string) => {
    if (/^\d?$/.test(value)) {
      ...
      // If value is entered, move focus to the next field
      ...
      // If value is empty (digit deleted), move focus to the previous field
      if (!value && index > 0) {
  ...
  return (
    <AuthCard title="Verify Now">
      <div className="flex justify-center gap-3 mb-3">
        {otp.map((digit, index) => (
          <input
            ...
            className="w-[5.125rem] h-[4.625rem]  bg-gray_light_3 text-center text-xl rounded-lg focus:ring-2 focus:ring-primary focus:outline-gray"
          />
        ))}
      </div>
      ...
      <Button
        text="Cancel"
        className="text-base bg-gray_light_3 text-black xl:text-[1.625rem] w-full mt-[2.125rem] md:mt-[1rem] xl:mt-[2.125rem] h-[48px] lg:[54px] xl:h-[66px]"
        bg="bg-gray_light_3"
