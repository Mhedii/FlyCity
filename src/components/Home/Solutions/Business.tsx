  ...
  return (
    <section className=" pt-16 pb-10 md:pb-28 bg-[#F7F9FC] text-center ">
      <div className=" w-full text-center justify-center mt-2 md:mt-32 flex ">
        <p
          className="text-[3.875rem] leading-[5.181rem] font-bold  mb-[3.875rem]  max-w-4xl    "
      ...
      <div className=" mx-[1rem] lg:mx-[5rem]  xl:mx-[13rem] 2xl:mx-[15.75rem]  items-center grid  grid-cols-1 md:grid-cols-2 ">
        ...
        <div className=" grid gap-6 grid-cols-1 md:grid-cols-2  ">
          {businessData.map((item, index) => (
            <div
              key={index}
              className="card  bg-white  rounded-3xl  pl-[1.875rem] pe-[10px] pt-[40px]    transition-transform hover:scale-105 hover:cursor-pointer text-start "
            >
              <img
                src={item.icon}
                ...
                alt=""
              />
              <h3 className="mt-8 text-2xl leading-[2.375rem] font-bold mb-[20px] ">
                {item.title}
              </h3>
              <p className="text-gray text-[1.063rem] leading-[1.825rem] max-h-[3.625rem] mb-[31px] ">
                {item.description}
              </p>
            </div>
          ))}
